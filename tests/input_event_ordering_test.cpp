...
std::vector<uint32_t> codes(
  ...
  for (const auto& input : events) result.push_back(input.code);
  return result;
}
...
void mixed_press_and_release_order_is_stable() {
  std::vector events{event(a_key, 1), event(left_shift, 0), event(b_key, 0)};
  reorder_same_report_events(events);
  assert((codes(events) == std::vector<uint32_t>{a_key, b_key, left_shift}));
}
